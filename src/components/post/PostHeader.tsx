...
export default function PostHeader({ publication }: Props) {
  ...
  return (
    <div className={styles.postHeaderContainer}>
      ...
      <h1>{publication?.publication?.metadata?.name}</h1>
      ...
      >
        <MediaRenderer
          ...
          alt={
            publication?.publication?.profile?.name || 'Author profile picture'
          }
        />
      </Link>
      ...
      <h1>
        <b>{formatNum(publication.publication?.stats.totalUpvotes)}</b> Upvotes
      </h1>
      <h1>
      ...
      <h1>
        <b>{formatNum(publication.publication?.stats.totalAmountOfCollects)}</b>{' '}
        Collects
      </h1>
      <h1>
        <b>{formatNum(publication.publication?.stats.totalAmountOfMirrors)}</b>{' '}
        Mirrors
      </h1>
    </div>
  );
